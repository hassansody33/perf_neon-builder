...
case "$DEVICE_IMPORT" in
    sweet|davinci|tucana|violet|ginkgo|laurel_sprout|sweet-pixelos|sweet-miui|sweet-clang)
        # Device specific for 4.14
        if [[ "$DEVICE_IMPORT" == "sweet" ]] || [[ "$DEVICE_IMPORT" == "sweet-pixelos" ]] || [[ "$DEVICE_IMPORT" == "sweet-clang" ]]; then
